...
export default class SortListItem extends Component
{
  ...
  snapToIndex(index, highlight)
  {
    ...
    Animated.timing(this.state.position, { toValue:newPos, duration: 300, }).start(() =>
    {
      ...
      if(highlight === true)
      {
        Animated.timing(this.state.position, { toValue:{x:35,y:0}, duration: 100, }).start(() =>
        {
          Animated.timing(this.state.position, { toValue:nullPos, duration: 100, }).start();
        });
      }
    });
  }
  ...
  render()
  {
    return (
      <Animated.View
        style={[
          s.draggable,
          this.props.horizontal === true ? {...G.S.height()} : {...G.S.width()},
          this.state.position.getLayout(),
          {zIndex:(this.state.isMoving === true ? 2 : 1)}
        ]}
        onLayout={(event) =>
        {
          const iSize = (this.props.horizontal === true ? event.nativeEvent.layout.width : event.nativeEvent.layout.height);
          this.setState({ ...this.state, itemSize:iSize });
          this.props.setItemSize(iSize);
        }}
      >
