...
export default function _05_EndDate(props)
{
  const [errorMessage, setErrorMessage] = React.useState("");
  const [date, setDate] = React.useState(props.trip?.endDate);
  const [showDatePicker, setShowDatePicker] = React.useState(false);
  const [dirty, setDirty] = React.useState(typeof props.trip?.endDate === 'undefined' || props.trip?.endDate === null);
  ...
  const saveAndNext = () =>
  {
    ...
    else if(G.Functions.daysBetween(props.trip.startDate, date) < 1)
      setErrorMessage("Your end date must be after your start date\n(Your start date is " + G.Functions.dateToText(props.trip.startDate, "MMM Do YYYY") +").");
    else if(dirty === false)
      props.next();
    else
      props.updateTrip({...props.trip, endDate: date});
  };

  const dateObj = G.Functions.toMoment(date === null ? props.trip.startDate : date);
  
  return (
    <View style={s.container}>
      <View style={s.content}>
        <Texts.Label style={s.message}>
          <Text style={[s.normalText, {fontSize: 16, color:G.Colors().Neutral(0.6)}]}>{"What would be your last day?\n\n\n"}</Text>
        </Texts.Label>
      ...
      <View style={s.actionContainer}>
        <View style={s.buttonContainer}>
          {getButton("Select my ending position", saveAndNext, false, false)}
        </View>
      </View>
      <Pickers.Date
        title={"End date"}
